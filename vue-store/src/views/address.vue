<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(list, index) in addressListFilter" :key="list.addressId" :class="{'check': checkIndex === index}" @click="checkIndex = index;addressSelectId = list.addressId">
                    <dl>
                      <dt>{{list.userName}}</dt>
                      <dd class="address">{{list.streetName}}</dd>
                      <dd class="tel">{{list.tel}}</dd>
                    </dl>
                    <div class="addr-opration addr-del" v-if="!list.isDefault">
                      <a href="javascript:;" class="addr-del-btn" @click="delAddressConfirm(list.addressId)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default" v-if="!list.isDefault" @click="setDefault(list.addressId)">
                      <a href="javascript:;" class="addr-set-default-btn"><i>Set default</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="list.isDefault">Default address</div>
                  </li>
                  <li class="addr-new">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path: 'orderConfirm', query: {addressId: addressSelectId}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal :mdShow="isMdShow" @close="closeModal">
        ...
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:void(0)" @click="delAddress">确认</a>
          <a class="btn btn--m" href="javascript:void(0)" @click="isMdShow = false">取消</a>
        </div>
      </modal>
  ...
  methods: {
    getAddressList () {
      axios.get('/users/addressList').then(res => {
        let data = res.data
        if (data.status === '0') {
          let defaultAddress = data.result.filter(x => {
            return x.isDefault
          })
          this.addressList = data.result.filter(x => {
            return !x.isDefault
          })
          this.addressSelectId = defaultAddress[0].addressId
    ...
    setDefault (addressId) {
      axios.post('/users/setDefault', {addressId}).then(res => {
        var data = res.data
        if (data.status === '0') {
          this.getAddressList()
        }
